function extlinkAttach(b){var h=b==document?$(document.body):$(b);if(h.closest(".sExtlink-processed").length){return}h.addClass("sExtlink-processed");var j=window.location.host.replace(/^(([^\/]+?\.)*)([^\.]{4,})((\.[a-z]{1,4})*)$/,"$3$4"),d=window.location.host.replace(/^(([^\/]+?\.)*)([^\.]{4,})((\.[a-z]{1,4})*)$/,"$1"),c=false,f=false,e,a;if(Drupal.settings.extlink.extSubdomains){e="([^/]*.)?"}else{if(d=="www."||d==""){e="(www.)?"}else{e=d.replace(".",".")}}a=new RegExp("^https?://"+e+j+"/","i");if(Drupal.settings.extlink.extInclude){c=new RegExp(Drupal.settings.extlink.extInclude.replace(/\\/,"\\"))}if(Drupal.settings.extlink.extExclude){f=new RegExp(Drupal.settings.extlink.extExclude.replace(/\\/,"\\"))}function g(k){var k=k.toLowerCase();if(k.indexOf("http")!==0){return false}if(k.match(a)&&!(c&&k.match(c))){return false}if(f&&k.match(f)){return false}return true}h.on("click","a",function(m){var k=$(this),l=this.href;if(k.hasClass("s-extlink-direct")){return}if(g(l)){m.preventDefault();
window.open("/link?path="+encodeURIComponent(k.attr("href")))}})}Drupal.behaviors.extlink=function(b){if(Drupal.settings.extlinkExtras&&Drupal.settings.extlinkExtras.disabled){return}if(typeof sAppLdbGetSecurityLevel=="undefined"){extlinkAttach(b)}else{if(typeof sAppLdbGetSecurityLevel!="undefined"&&sAppLdbGetSecurityLevel()!="restricted"){extlinkAttach(b)}}var a=sCommonGetSetting("extlink","mailtoClass");if(a){$("a:not(.sExtlink-processed)",b).addClass("sExtlink-processed").each(function(){var d=$(this),c=d.attr("href")||"";if(c.indexOf("mailto:")===0&&d.find("img:first").length===0){d.addClass(a);if(d.css("display")=="inline"){d.after("<span class="+a+"></span>")}}})}};Drupal.behaviors.popups=function(b){Popups.saveSettings();var c=$("body");if(!c.hasClass("popups-processed")){c.addClass("popups-processed");$(document).bind("keydown",Popups.keyHandle);var a=$("#popit");if(a.length){a.remove();Popups.message(a.html())}}if(Drupal.settings.popups&&Drupal.settings.popups.links){jQuery.each(Drupal.settings.popups.links,function(e,d){Popups.attach(b,e,Popups.options(d))
...
if(b){if(!b.options){if(!c){c={}}var g=Popups.options(c);b.options=g}}if(Popups.activePopup()){if(b.options.hideActive){Popups.activePopup().hide()}}if(!b){b=new Popups.Popup()}Popups.push(b);var f=b.fill(h,a,e);b.hide();if(d){f.css("width",d)}if(b.extraClass instanceof Array){b.extraClass=b.extraClass[b.extraClass.length-1]}f.addClass(b.extraClass);$("body").append(f);if(e){jQuery.each(e,function(k,j){$("#"+k).click(j.func)})}b.$popupClose().click(function(){return Popups.close(b)});Popups.resizeAndCenter(b);b.refocus();Popups.removeLoading();$(document).trigger("popups_open_done",[b]);b.element=b.element!=null?b.element:$("body")[0];Drupal.sAccessibility.setLastFocus($(b.element));Drupal.sAccessibility.hideFromAT($("#body"));b.trapTabKey();return b};Popups.resizeAndCenter=function(a){var g=a.$popup();if(a&&a.fullScreen){g.css({width:"100%",height:"100%",top:0,left:0});$("html").css("overflow","hidden");return}var e=g.width();var b=Popups.windowWidth();var d=Math.floor((b/2)-(e/2)+Popups.scrollLeft());
